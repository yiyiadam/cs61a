...
def roll_dice(num_rolls, dice=six_sided):
    ...
    while(num_rolls > 0):
        temp = dice()
        if temp == 1:
            sum_zero_cnt  = sum_zero_cnt +  temp
        else:
            sum_none_zero = sum_none_zero + temp

        num_rolls = num_rolls -1
    if sum_zero_cnt != 0:
        return sum_zero_cnt
    else:
        return sum_none_zero
    # END PROBLEM 1
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    return min(current_score,25-num_rolls)
...
def make_averaged(fn, num_samples=1000):
    ...
    def avfn(*arg):
        ...
        while count <num_samples:
            temp_average = (temp_average + fn(*arg) )
            count +=1
        return temp_average/num_samples
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    # BEGIN PROBLEM 8
    "*** REPLACE THIS LINE ***"
    test_dice ,num_dice = 1 ,1 
    avfn = make_averaged(roll_dice)
    highest = avfn(test_dice,dice)
    while test_dice <=10:
        current_hiscore = avfn(test_dice,dice)
        if highest < current_hiscore:
            highest = current_hiscore 
            num_dice = test_dice
        test_dice += 1
    return num_dice
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
    ...
    if opponent_score == 0:
        ...
        if score/opponent_score != 2 and turn_score >= margin:
            return 0
        else:
            return num_rolls
    # END PROBLEM 10
